...
public class ZombieBoss extends CustomEntity<Zombie> implements Attacker, Targeter{
	...
	//public ModeledEntity zombiebossentity;
	//public ActiveModel zombiebossmodel;
...
//		}
	...
	protected Zombie createEntity(Location place) {
		...
		//zombiebossentity = ModelEngineAPI.api.getModelManager().createModeledEntity(zombie);
		//zombiebossmodel = ModelEngineAPI.api.getModelManager().createActiveModel("wizpiglin");
		//zombiebossentity.addActiveModel(zombiebossmodel);
		//zombiebossentity.setInvisible(true);
		//zombiebossentity.getNametagHandler().setCustomName("tag_name", "Zombie Boss");
		//zombiebossentity.getNametagHandler().setCustomNameVisibility("tag_name", true);
		return zombie;
	}
...
//		}
	...
	public void attack(Damageable entity) {
		...
		if (attackState == 0) {
			//Spawn Minions
			//ModelEngineAPI.api.getModelManager().getModeledEntity(getEntity().getUniqueId()).getActiveModel("wizpiglin").addState("riftself",1,1,1.0);
			Location place = getEntity().getLocation();
			createMinions(place);
		}
		else {
			//Hit
			//ModelEngineAPI.api.getModelManager().getModeledEntity(getEntity().getUniqueId()).getActiveModel("wizpiglin").addState("kick",1,1,1.0);
