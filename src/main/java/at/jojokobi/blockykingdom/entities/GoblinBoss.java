...
public class GoblinBoss extends CustomEntity<Zombie> implements Attacker {
	...
	//public ModeledEntity goblibossnentity;
	//public ActiveModel goblibossnmodel;
	...
	protected Zombie createEntity(Location place) {
		...
		//goblibossnentity.addActiveModel(goblibossnmodel);
		//goblibossnentity.setInvisible(true);
		//goblibossnentity.getNametagHandler().setCustomName("tag_name", "Goblin Boss");
		//goblibossnentity.getNametagHandler().setCustomNameVisibility("tag_name", true);
		return entity;
	}
	...
	public void attack(Damageable entity) {
		if (getEntity().getLocation().distance(entity.getLocation()) > 4 || !EntityUtil.canHit(getEntity(), entity)) {
			...
			//ModelEngineAPI.api.getModelManager().getModeledEntity(getEntity().getUniqueId()).getActiveModel("goblinking").addState("attack2",1,1,1.0);
		} else {
			entity.damage(16);
			//ModelEngineAPI.api.getModelManager().getModeledEntity(getEntity().getUniqueId()).getActiveModel("goblinking").addState("attack1",1,1,1.0);
		}
